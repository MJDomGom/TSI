...
class Envio(models.Model):
    ...
    state = fields.Selection([('preparando', 'Preparando'), ('enviado', 'Enviado'), ('reparto', 'En reparto'), (
        'entregado', 'Entregado'), ('cancelado', 'Cancelado')], 'Estado', default='preparando')
    # Añadir las relaciones entre clases
    venta_id = fields.Many2one(
        "upo_wood_app.venta", string="Venta relacionada con el envio")
    albaran_id = fields.Many2one(
        "upo_wood_app.albaran", string="Albaran relacionado con el envio")
    _sql_constraints = [('envio_name_unique', 'UNIQUE (name)',
                         'El número del envío debe ser único')]

    # Funciones para el workflow de envio
